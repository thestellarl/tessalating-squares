...
checkExpr x (Lit i)   = True 
checkExpr x (Ref r)   = elem r x
checkExpr x (Add l r) = and (map (checkExpr x) [l, r])
checkExpr x (Mul l r) = and (map (checkExpr x) [l, r])
...
checkCmd m v (Pen _) = True
checkCmd m v (Move l r)     = and (map (checkExpr v) [l, r])
checkCmd m v (For a b c d)  = and ( (checkBlock m (a: v) d) : map (checkExpr v) [b, c])
checkCmd m v (Call b a)     = case (and (map (checkExpr v) a), get b m) of
                                  (True, Just c) -> length a == c
                                  _              -> False
...
checkBlock m v x = and (map (checkCmd m v) x)
...
checkDef m (Define macro p b) = and (map (checkCmd m p) b)
...
--
expr :: Env -> Expr -> Int
expr v (Ref a)   = getOrFail a v
expr v (Lit i)   = i
expr v (Add l r) = (expr v l) + (expr v r)
expr v (Mul l r) = (expr v l) * (expr v r)
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Move xExp yExp -> case pen of
      Up   -> ((pen, nPos), [])
      Down -> ((pen, nPos), [(pos, nPos)])
      where nPos = (expr env xExp, expr env yExp)
        

    Call macro args -> case getOrFail macro defs of 
      (params, b) -> block defs (setAll params (map (expr env) args) env) state b
...
nestedBox :: Def
nestedBox = Define "nestedBox" ["x1", "y1", "h", "w", "n", "d"]
  [
    For "i" (Lit 1) (Ref "n")
      [ Call "box" [Ref "x1", Ref "y1", Mul (Ref "w") (Mul (Ref "d") (Ref "i")), Mul (Ref "h") (Mul (Ref "d") (Ref "i"))] ]
  ]
...
rep = Define "rep" ["y", "n"]
  [
    For "i" (Lit 0) (Ref "n")
      [Call "nestedBox" [Mul (Ref "i") (Lit 8), (Ref "y"), Lit 1, Lit 1, Lit 4, Lit 1]
      , Call "nestedBox" [Add (Lit 8) (Mul (Ref "i") (Lit 8)), Add (Lit (4))(Ref "y"), Lit 1, Lit 1, Lit 4, Lit (-1)]
      ]
  ]

-- | A MiniLogo program that draws your amazing picture.
amazing :: Prog
amazing = Program [nestedBox, box, rep ] 
  [
    For "i" (Lit 0) (Lit 9)
    [
      Call "rep" [Mul (Lit 4) (Ref "i"), Lit 9]
    ]
  ]
